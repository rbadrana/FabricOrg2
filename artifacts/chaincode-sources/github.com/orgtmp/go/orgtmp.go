...
func (s *SmartContract) Invoke(stub shim.ChaincodeStubInterface) sc.Response {
	...
	if function == "regOrgSensor" {
		return s.regOrgSensor(stub, args)
	} else if function == "setOrgTmp" {
		return s.setOrgTmp(stub, args)
	} else if function == "getOrgTmp" {
		return s.getOrgTmp(stub, args)
	}
	return shim.Success(nil)
}

func (s *SmartContract) regOrgSensor(stub shim.ChaincodeStubInterface, args []string) sc.Response {
...
func (s *SmartContract) setOrgTmp(stub shim.ChaincodeStubInterface, args []string) sc.Response {
...
func (s *SmartContract) getOrgTmp(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	if len(args) != 1 {
		return shim.Error("invalid number of arguments")
	}
	sensorID := args[0]
